...
export function caseInsensitiveObject(obj: any): any {
  ...
  return new Proxy(proxy, {
    deleteProperty: (tgt: any, k: string): boolean => delete tgt[normalize(k)],
    get: (tgt: any, k: string): any => tgt[normalize(k)],
    set: (tgt: any, k: string, v: any): boolean => {
      tgt[normalize(k)] = v;
      return true;
    }
  });
...
export function fromReadableStream(stream: Readable): Observable<any> {
  ...
  return new Observable((observer) => {
    const next = (chunk): number => buffer.push(chunk);
    const error = (err): void => observer.error(err);
    const complete = (): void => {
